...
const makeCard = (overrides?: Partial<SwipeCard>): SwipeCard => ({
  ...
  character: 'boss',
  text: { zh: '测试', en: 'test' },
  leftChoice: { label: { zh: '拒绝', en: 'reject' }, effects: { stability: 5 } },
  rightChoice: { label: { zh: '接受', en: 'accept' }, effects: { stability: -10, skill: 15 } },
  stage: 1,
  scene: 'universal',
  ...overrides,
});
...
const makeState = (attrs?: Partial<Attributes>): GameState => ({
  careerId: 'programmer',
  currentCard: 0,
  attributes: { stability: 50, skill: 50, finance: 50, network: 50, sanity: 50, ...attrs },
  history: [],
  isGameOver: false,
  seed: 1,
  deck: [makeCard()],
...
describe('applySwipe', () => {
  it('applies left choice effects and clamps to 0-100', () => {
    const state = makeState({ stability: 98 });
    const { newState } = applySwipe(state, makeCard({ leftChoice: { label: { zh: '左', en: 'l' }, effects: { stability: 10 } } }), 'left');
    expect(newState.attributes.stability).toBe(100);
  ...
  it('applies right choice effects', () => {
    ...
    expect(newState.attributes.stability).toBe(40);
    expect(newState.attributes.skill).toBe(65);
  });

  it('clamps attributes to minimum 0', () => {
    const state = makeState({ stability: 5 });
    const { newState } = applySwipe(state, makeCard({ rightChoice: { label: { zh: '右', en: 'r' }, effects: { stability: -20 } } }), 'right');
    expect(newState.attributes.stability).toBe(0);
  });
...
describe('checkGameOver', () => {
  it('returns null when all attributes in range', () => {
    expect(checkGameOver({ stability: 50, skill: 50, finance: 50, network: 50, sanity: 50 }, 15)).toBeNull();
  });

  it('detects attr_zero', () => {
    const result = checkGameOver({ stability: 0, skill: 50, finance: 50, network: 50, sanity: 50 }, 5);
    expect(result).toEqual({ reason: 'attr_zero', attr: 'stability' });
  });

  it('detects attr_max', () => {
    const result = checkGameOver({ stability: 50, skill: 100, finance: 50, network: 50, sanity: 50 }, 5);
    expect(result).toEqual({ reason: 'attr_max', attr: 'skill' });
  });

  it('detects won at card 30', () => {
    const result = checkGameOver({ stability: 50, skill: 50, finance: 50, network: 50, sanity: 50 }, 30);
    expect(result).toEqual({ reason: 'won' });
  });

  it('detects sanity hitting zero', () => {
    const result = checkGameOver({ stability: 50, skill: 50, finance: 50, network: 50, sanity: 0 }, 5);
    expect(result).toEqual({ reason: 'attr_zero', attr: 'sanity' });
  });

  it('detects sanity hitting max', () => {
    const result = checkGameOver({ stability: 50, skill: 50, finance: 50, network: 50, sanity: 100 }, 5);
    expect(result).toEqual({ reason: 'attr_max', attr: 'sanity' });
  });
});
